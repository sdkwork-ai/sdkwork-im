...
import type { Moment, MomentPreviewState } from "../../types";

interface MomentItemCardProps {
  moment: Moment;
  /** Signed-in user id; only the author may delete a moment. */
  currentUserId?: string;
  /** Pre-composed like summary, e.g. "Alex Chen，以及 3 人". */
  likedByLabel: string;
  ...
  setPreviewState: (state: MomentPreviewState) => void;
...
export const MomentItemCard: React.FC<MomentItemCardProps> = ({
  ...
  const isMine = moment.author.id === currentUserId;
  ...
  return (
    <div className="flex gap-3 px-4 py-4 border-b border-black/5 dark:border-white/5 bg-chat-other-bg active:bg-hover-bg transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        {(moment.likeCount > 0 || moment.comments.length > 0) && (
            ...
            <div className="relative z-10 p-2.5 space-y-1.5">
              {moment.likeCount > 0 && (
                <div className="flex items-start gap-1.5 text-[#576B95] dark:text-[#7d90a9] font-medium leading-relaxed">
                  ...
                  <span className="break-words">{likedByLabel}</span>
                </div>
              )}
