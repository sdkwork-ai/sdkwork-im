    ...
    <div className="w-full h-full bg-[#121212] overflow-y-auto pb-[60px] relative">
       ...
       <div className="w-full relative pt-safe bg-gradient-to-b from-[#2B2B2B] to-[#121212]">
             ...
             <div className="flex-1 mt-1">
                ...
                <div className="flex items-center gap-6">
                   <div className="flex flex-col">
                      ...
                      <span className="text-[12px] text-white/50">{t('channels.auto_n7e715800', 'Likes & Favorites')}</span>
                   </div>
                   <div className="flex flex-col">
                      <span className="font-bold text-white text-[16px]">42</span>
                      <span className="text-[12px] text-white/50">{t('channels.auto_a49d5', 'Follow')}</span>
                   </div>
                   <div className="flex flex-col">
                      <span className="font-bold text-white text-[16px]">1.2w</span>
                      <span className="text-[12px] text-white/50">{t('channels.auto_f62b4', 'Followers')}</span>
                   </div>
                </div>
             </div>
          </div>
          ...
          <div className="px-4 pb-4">
             <p className="text-[13px] text-white/80 leading-relaxed max-w-[90%]">{t('channels.auto_444be3cd', 'Building the future with code and prompts. I love exploring cutting-edge AIGC technology — let\'s connect!')}<br />
               <span className="text-white/40 flex items-center gap-1 mt-1">
                 <MapPin className="w-3 h-3" />{t('channels.auto_30928e61', 'Starry Sea')}</span>
             </p>
             <div className="flex gap-2 mt-4">
                <button className="flex-1 h-9 rounded bg-white/10 text-white font-medium text-[14px]">{t('channels.auto_3bf19150', 'Edit profile')}</button>
                <button className="flex-1 h-9 rounded bg-white/10 text-white font-medium text-[14px]">{t('channels.auto_334184e5', 'Add friends')}</button>
             </div>
          </div>
       </div>
       ...
       <div className="sticky top-0 bg-[#121212]/95 backdrop-blur-md z-10 flex border-b border-white/5 px-2 pt-2">
          ...
          <TabItem icon={Wand2} label={t('channels.auto_prop_3e900dbe', 'Prompts 12')} active={activeTab === 'prompts'} onClick={() => setActiveTab('prompts')} />
          <TabItem icon={Heart} label={t('channels.auto_prop_n30ed3358', 'Favorites 45')} active={activeTab === 'favorites'} onClick={() => setActiveTab('favorites')} />
          <TabItem icon={Lock} label={t('channels.auto_prop_f1a25', 'Private')} active={activeTab === 'private'} onClick={() => setActiveTab('private')} />
       </div>
