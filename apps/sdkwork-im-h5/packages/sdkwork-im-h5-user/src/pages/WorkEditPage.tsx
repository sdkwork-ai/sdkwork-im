...
const { id } = useParams<{ id: string }>();
  ...
  const handleSave = async () => {
    if (!title.trim()) {
      showToast(t('user.auto_fn_be5b58b', 'Please enter a work title'));
      return;
    }
    if (work) {
      try {
  ...
  if (unavailable || !work) return (
     <PageLayout title={t('user.auto_prop_3bea0160', 'Edit work')}>
       <div className="flex-1 flex items-center justify-center text-text-sub">{t('user.auto_n243721ff', 'Work not found')}</div>
     </PageLayout>
  );

  return (
    <PageLayout 
       title={t('user.auto_prop_3bea0160', 'Edit work')}
       rightElement={
          <button 
             onClick={handleSave}
    ...
    >
      <div className="p-4 flex flex-col gap-6">
        <div className="flex flex-col gap-2">
           <label className="text-[14px] text-text-sub font-medium">{t('user.auto_255d0726', 'Cover')}</label>
           <div 
           ...
           >
             {coverUrl ? (
                ...
                <div className="flex-1 h-full flex items-center justify-center text-text-sub text-[13px]">{t('user.auto_1f270253', 'Tap to set cover')}</div>
             )}
             <div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                <span className="text-white text-[13px] font-medium">{t('user.auto_304d92ef', 'Change cover')}</span>
             </div>
           </div>
        </div>
        
        <div className="flex flex-col gap-2">
           <label className="text-[14px] text-text-sub font-medium">{t('user.auto_255e7d16', 'Work title')}</label>
           <input 
              value={title}
              ...
              placeholder={t('user.auto_prop_69a34051', 'Give your work a great name')}
           />
        </div>
      </div>
    </PageLayout>
