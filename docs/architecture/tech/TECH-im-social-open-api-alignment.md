...
- **Conversation roster persistence**: conversation creation paths (`create_conversation`, `bind_direct_chat`, `create_thread`, `create_agent_dialog`, `create_system_channel`, `create_agent_handoff`), membership mutations (add/remove/leave/linked-member sync), role/owner changes (`change_conversation_member_role`, `transfer_conversation_owner`), and read-cursor updates all call `persist_aggregate_state()` / `best_effort_persist_aggregate_state()` when `PostgresAggregateStore` is configured via conversation-runtime bootstrap. `im_projection_conversation_members` and `im_projection_read_cursors` are command-side materialized projections whose write owner is `comms-conversation-service` (declared in `specs/database-table-registry.json`); `projection-service` never writes them. RTC/session-gateway authorization and message search read these tables and require members to be visible immediately after creation, so persist runs before `maybe_evict_after_write`.
