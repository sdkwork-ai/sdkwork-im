...
- [开发环境PostgreSQL数据库配置教程](./开发环境PostgreSQL数据库配置教程.md)
  ...
  - Uses split `SDKWORK_IM_DATABASE_*` host/engine/ssl fields plus unified `SDKWORK_IM_DATABASE_*` workspace identity (`NAME`, `SCHEMA`, `USERNAME`, `PASSWORD`), and `SDKWORK_DATABASE_ADMIN_PASSWORD` for bootstrap.
