...
<div class="api-overview-grid">
  ...
  <div class="api-card">
    ...
    <p><code>/im/v3/api</code> runtime routes for Realtime Presence, realtime delivery, chat, media, and RTC.</p>
  ...
  <div class="api-card">
    <h3>Backend API</h3>
    <p><code>/backend/v3/api</code> management, operator, audit, and control endpoints.</p>
...
- For packaged installs, start with [Gateway OpenAPI](/api-reference/gateway-openapi):
  ...
  `/backend/v3/api/*` discovery all converge on the same unified public origin.
...
- SDK families remain boundary-specific: `sdkwork-im-sdk` maps to `/im/v3/api`,
  ...
  `/backend/v3/api/control/*` is a backend SDK module.
...
- Use [Backend SDK](/sdk/backend-sdk) when you need ops, audit, or control-plane client boundaries.
