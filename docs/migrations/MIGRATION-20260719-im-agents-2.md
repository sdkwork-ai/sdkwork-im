...
2. Assignment state, binding, durable dispatch, lease recovery, timeout reconciliation,
...
Any future in-place conversion of an existing TEXT subject column requires a
separate reviewed migration. No legacy TEXT integration column exists in `0005`,
so no compatibility column, shadow table, dual write, or destructive backfill is
