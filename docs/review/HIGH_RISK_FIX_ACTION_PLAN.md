  ...
  security-gate:
    ...
    steps:
      ...
      - name: Check Runtime Profile
        run: |
          PROFILE=$(grep SDKWORK_IM_RUNTIME_PROFILE etc/topology/cloud.production.env | cut -d'=' -f2)
          if [ "$PROFILE" != "production" ]; then
      ...
      - name: Check JWT Signature
        run: |
          JWT_SIG=$(grep SDKWORK_IM_APP_CONTEXT_REQUIRE_SIGNATURE etc/topology/cloud.production.env | cut -d'=' -f2)
          if [ "$JWT_SIG" != "true" ]; then
      ...
      - name: Check IAM Database
        run: |
          if ! grep -q "SDKWORK_DATABASE_URL" etc/topology/cloud.production.env; then
...
// 文件: etc/topology/cloud.production.env
