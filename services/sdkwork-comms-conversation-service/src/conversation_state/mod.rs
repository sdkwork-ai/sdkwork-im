...
impl ConversationStateService {
    pub fn reset_for_recovery(&self) {
        lock_conversation_state_mutex(&self.entries, "conversation_state store").clear();
        lock_conversation_state_mutex(
            &self.message_conversation_index,
            "message conversation index",
        )
        .clear();
        lock_conversation_state_mutex(&self.summaries, "summary store").clear();
        lock_conversation_state_mutex(&self.members, "member store").clear();
        lock_conversation_state_mutex(&self.read_cursors, "cursor store").clear();
        lock_conversation_state_mutex(&self.received_messages, "received message index").clear();
        lock_conversation_state_mutex(&self.conversations, "conversation store").clear();
        lock_conversation_state_mutex(
            &self.group_conversation_bindings,
            "group conversation binding store",
        )
        .clear();
        lock_conversation_state_mutex(&self.contacts, "contact store").clear();
        lock_conversation_state_mutex(
