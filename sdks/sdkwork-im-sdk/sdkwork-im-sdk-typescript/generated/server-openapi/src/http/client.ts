...
export class HttpClient extends BaseHttpClient {
  ...
  constructor(config: SdkworkImConfig) {
    HttpClient.assertInitialCredentialMode(config as any);
    super(config as any);
  ...
  private static assertInitialCredentialMode(config: any): void {
  ...
  private applySdkworkAuthHeaders(headers?: Record<string, string>): Record<string, string> | undefined {
    ...
    const apiKey = HttpClient.normalizeCredential(authConfig.apiKey);
    if (apiKey) {
      if (authToken || accessToken) {
        throw new Error(
...
export function createHttpClient(config: SdkworkImConfig): HttpClient {
