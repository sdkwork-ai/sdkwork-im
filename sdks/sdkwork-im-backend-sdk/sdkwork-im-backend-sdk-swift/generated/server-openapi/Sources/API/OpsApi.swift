...
public class OpsApi {
    ...
    /// Retrieve ops health
    public func healthRetrieve() async throws -> HealthRetrieveResponse? {
        return try await client.get(ApiPaths.backendPath("/ops/health"), responseType: HealthRetrieveResponse.self)
    }

    /// Retrieve cluster state
    public func clusterRetrieve() async throws -> ClusterRetrieveResponse? {
    ...
    public func providerBindingsList(pageSize: Int? = nil, cursor: String? = nil) async throws -> ProviderBindingSnapshotListResponse? {
        let query = buildQueryString([
            QueryParameterSpec(name: "page_size", value: pageSize, style: "form", explode: true, allowReserved: false, contentType: nil),
            QueryParameterSpec(name: "cursor", value: cursor, style: "form", explode: true, allowReserved: false, contentType: nil)
        ])
        return try await client.get(ApiPaths.appendQueryString(ApiPaths.backendPath("/ops/provider_bindings"), query), responseType: ProviderBindingSnapshotListResponse.self)
    }

    /// Retrieve provider binding drift
    public func providerBindingsDriftRetrieve(pageSize: Int? = nil, cursor: String? = nil) async throws -> ProviderBindingDriftListResponse? {
        let query = buildQueryString([
            QueryParameterSpec(name: "page_size", value: pageSize, style: "form", explode: true, allowReserved: false, contentType: nil),
            QueryParameterSpec(name: "cursor", value: cursor, style: "form", explode: true, allowReserved: false, contentType: nil)
        ])
        return try await client.get(ApiPaths.appendQueryString(ApiPaths.backendPath("/ops/provider_bindings/drift"), query), responseType: ProviderBindingDriftListResponse.self)
    }
