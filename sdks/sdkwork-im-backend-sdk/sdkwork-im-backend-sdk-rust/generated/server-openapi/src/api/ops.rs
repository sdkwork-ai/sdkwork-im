...
impl OpsApi {
    ...
    pub async fn provider_bindings_list(&self, page_size: Option<i64>, cursor: Option<&str>) -> Result<ProviderBindingSnapshotPageData, SdkworkError> {
        let query = build_query_string(&[
            QueryParameterSpec::new("page_size", page_size, "form", true, false, None),
            QueryParameterSpec::new("cursor", cursor, "form", true, false, None),
        ]);
        let path = append_query_string(backend_path(&"/ops/provider_bindings".to_string()), &query);
        self.client.get(&path, None, None).await
    }

    /// Retrieve provider binding drift
    pub async fn provider_bindings_drift_retrieve(&self, page_size: Option<i64>, cursor: Option<&str>) -> Result<ProviderBindingDriftPageData, SdkworkError> {
        let query = build_query_string(&[
            QueryParameterSpec::new("page_size", page_size, "form", true, false, None),
            QueryParameterSpec::new("cursor", cursor, "form", true, false, None),
        ]);
        let path = append_query_string(backend_path(&"/ops/provider_bindings/drift".to_string()), &query);
        self.client.get(&path, None, None).await
    }
