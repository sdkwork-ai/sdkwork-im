...
echo "== PATCH /iam/users/current {displayName} =="
curl --noproxy '*' -s -X PATCH http://127.0.0.1:18079/app/v3/api/iam/users/current \
  -H "Authorization: Bearer $AUTH" -H "access-token: $AT" -H 'Content-Type: application/json' \
  -d '{"displayName":"Administrator"}' | head -c 300
echo
